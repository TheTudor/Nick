...
public class EyeXHost : MonoBehaviour
{
    ...
    private Vector2 _gameWindowPosition;
    ...
    private void OnConnectionStateChanged(object sender, ConnectionStateChangedEventArgs e)
    {
        ...
        if ((e.State == ConnectionState.Connected) && (_globalInteractors.Count > 0))
        {
            _isConnected = true;
    ...
    {
        if (result.ResultCode == SnapshotResultCode.InvalidSnapshot)
        {
            Debug.LogWarning("Could not commit snapshot: " + result.ErrorMessage);
        }
        else if (result.ResultCode != SnapshotResultCode.Ok)
        {
            Debug.LogWarning("Could not commit snapshot: " + result.ErrorMessage);
        }
    }
    ...
    private void HandleQuery(InteractionQuery query)
    {
        ...
        try
        {
            ...
            // Get query bounds and map them to game coordinates.
            ...
            var queryRectInGameCoordinates = new Rect(
            ...
            {
                if (interactor.IntersectsWith(queryRectInGameCoordinates))
                {
    ...
    private void HandleEvent(InteractionEvent @event)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    globalInteractor.HandleEvent(@event);
                }
            }
        }
        catch (InteractionApiException ex)
...
public delegate void EyeXEventHandler(string interactorId, InteractionEvent @event);
