...
public sealed class EyeXGazePointDataStream : IDisposable
{
    ...
    private void AddFixationDataBehavior(Interactor interactor, FixationDataMode fixationDataMode)
    {
        ...
        behavior.SetFixationDataOptions(ref behaviorParams);
    }

    private void HandleEvent(string interactorid, InteractionEvent @event)
    {
        ...
        foreach (var behavior in @event.Behaviors)
        {
            ...
            else if (behavior.BehaviorType == InteractionBehaviorType.FixationData)
            {
                ...
                {
                    if (fixationEventParams.EventType == FixationDataEventType.Start)
                    {
                        FixationCount++;
                    }
                    else if (fixationEventParams.EventType == FixationDataEventType.Stop)
                    {
