...
public struct ProjectedRect
{
    ...
    /// <summary>
    /// The bounding rectangle in x-y space.
    /// </summary>
    public Rect rect;
    ...
    public static ProjectedRect GetProjectedRect(GameObject gameObject, Camera camera)
    {
        ...
        bool first = true;
        foreach (var point in GetBoundingBoxCornerPoints(gameObject))
        {
        ...
        {
            // Use mean z value. Not always correct, but an ok approximation
            var zMean = (zMin + zMax) / 2.0f;
            return new ProjectedRect { isValid = true, rect = new Rect(xMin, yMin, xMax - xMin, yMax - yMin), relativeZ = camera.farClipPlane - zMean };
    ...
    private static IEnumerable<Vector3> GetBoundingBoxCornerPoints(GameObject gameObject)
    {
        var boundingBox = gameObject.renderer.bounds;
